 ...
 -->   </xsl:template>
   ...
   <xsl:call-template name="COMMENT_FIELD"/>
            <xsl:choose>
      		  ...
      		  <xsl:text>&#9;&#9;AAAassertField(ids.</xsl:text><xsl:value-of select="concat($path, '.', @name)"/><xsl:text>, "</xsl:text><xsl:value-of select="ancestor::IDS/@name"/><xsl:text>/</xsl:text><xsl:value-of select="@path"/><xsl:text>");&#10;</xsl:text>
        	</xsl:when>
	        <xsl:otherwise>
			<xsl:text>&#9;&#9;BBBassertField(ids.</xsl:text><xsl:value-of select="concat($path, '.', @name)"/><xsl:text>, "</xsl:text><xsl:value-of select="ancestor::IDS/@name"/><xsl:text>/</xsl:text><xsl:value-of select="@path"/><xsl:text>");&#10;</xsl:text>
     	        </xsl:otherwise>
            </xsl:choose>
        </xsl:for-each>
        <xsl:for-each select="field[@data_type='structure']">
