...
int main(int argc, char *argv[])
{
  ...
  for(i=0; i < Sz; i++) {
    ...
    for (j=0; j<=i;j++)
      ids._core_profiles.profiles_1d(i).grid.rho_tor_norm(j) = vect1DDouble_1[j];
    ...
    for (j=0;j<= i; j++) {
      ...
      for (int k=0; k<=i; k++)
	ids._core_profiles.profiles_1d(i).ion(j).density(k) = vect1DDouble_1[k]+j;
  ...
  ids._core_profiles.ids_properties.homogeneous_time = 0; //! Mandatory to define this property
  ids._core_profiles.ids_properties.comment = "This is a test ids V3 Put by C++";
  ids._core_profiles.global_quantities.ip.resize(sizeof(time_2)/sizeof(time_2)[0]);
