...
void execute(char** argv) {
  ...
  if (assertion[1]) {
    for (int i = 0; i < ids.profiles_1d.size()/2; i++) {
      assertion[2*i + 2] = ids.profiles_1d(2*i).time == (double) (2*i) + 1.0;
      assertion[2*i + 3] = std::abs(ids.profiles_1d(2*i+1).time + 9e40) < epsilon;
    }
  }
  ...
  for (int i=3; i < ids.profiles_1d.size(); i++)
    assertion[i+2] = std::abs(ids.profiles_1d(i).time + 9e40) < epsilon;
